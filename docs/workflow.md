...
**シート構成:**
- 各イベント名をシート名として出力
- Stellaメタデータ、ステージマスタ、グループマスタ、出番データを左から横並びで配置（ブロック開始列は前ブロックの列数から動的に算出するため、列が増えても重ならない）
- 先頭の Stellaメタデータブロックは `JSON生成シート.xlsm` の【共通】相当（ライブ名 / 公演日 / liveId / bundleId / JSONバージョン / 開始時 / 終了時 / お知らせバージョン / お知らせメッセージ）。値は保存済みの `stella_metadata` / `stella_project_meta` を出力
- ステージマスタは Stella JSON の stageList と同等の詳細（`ステージ名_短縮` / `カラー名` / `表示順`）を含み、`表示順` 昇順で出力。非活性化ステージは除外
- 出番データのコラボ出番（同一 `出番ID` の複数行）は **1 行に統合** し、`グループID` / `グループ名` / `グループ名(OCR)` をカンマ区切りで連結（例: グループID `3,5`）
