...
def _sync_to_session(state: AppState) -> None:
    ...
    注意: crop_tgt_event / crop_tgt_img_type / ocr_tgt_event / ocr_tgt_img_type
    は ③/④ の selectbox の key であり、ここで直接書くと
    Streamlit の widget レジストリに焼き込まれてユーザー操作で上書きできなくなる。
    これらは ③/④ 各 render 関数の pre-init ブロックで app_state から初期化する。
    """
...
def determine_timetable_image():
    ...
    if last_dir_name is not None:
        ...
        # widget key を直接書くと Streamlit の widget レジストリに焼き込まれて
        # 以後ユーザー操作で上書きできなくなるため、pop だけして
        # 次回 ③/④ render 時に pre-init で app_state から読ませる。
        for _k in ("crop_tgt_event", "crop_tgt_img_type",
                   "ocr_tgt_event", "ocr_tgt_img_type"):
...
def set_crop_image():
    ...
    # イベント/種別切替時は前イベント由来の derived state を全て破棄する。
    app_state.crop.cropped_image = None
    app_state.crop.crop_box = None
    app_state.crop.annotated_image = None
    app_state.crop.stage_line_list = None
    app_state.crop.images_eachstage = []
    app_state.crop.images_eachstage_bbox = []
    app_state.crop.stage_crop_rects = []
    # session_state 側 (widget key ではないので pop ではなく初期値で上書き)
    st.session_state.cropped_image = None
    st.session_state.crop_box = None
    st.session_state.stage_line_list = None
...
def set_ocr_image():
    ...
    # イベント/種別切替時は前イベント由来の derived state を全て破棄する。
    app_state.ocr.ocr_tgt_image_info = None
    app_state.ocr.ocr_tgt_stage_num = 0
    app_state.ocr.df_timetables = []
    app_state.ocr.time_axis_detect = None
    app_state.ocr.timeline_eachstage = []
    # session_state 側 (widget key ではないので pop ではなく初期値で上書き)
    st.session_state.ocr_tgt_image_info = None
    st.session_state.ocr_tgt_stage_num = 0
...
def generate_change_proposal_for_stage(stage_no):
    """⑤変更比較：1ステージの変更案をLLM生成しsession_stateに保持する。"""
    diff_result = st.session_state.get("_diff_result")
    if diff_result is None:
        return
    result = _image_wf.propose_changes_for_stage(
        app_state, st.session_state.diff_tgt_event,
        st.session_state.diff_tgt_img_type, stage_no, diff_result,
    )
    proposals = st.session_state.setdefault("_change_proposals", {})
    if not result.success:
        st.session_state["_change_proposal_msg"] = result.error
        return
    proposals[stage_no] = result.data


def generate_change_proposal_all(diff_stages):
    """⑤変更比較：差分ありステージ（タイテ有り）の変更案を一括生成する。"""
    diff_result = st.session_state.get("_diff_result")
    if diff_result is None:
        return
    result = _image_wf.propose_changes_all_diff_stages(
        app_state, st.session_state.diff_tgt_event,
        st.session_state.diff_tgt_img_type, diff_stages, diff_result,
    )
    if not result.success:
        st.session_state["_change_proposal_msg"] = result.error
        return
    proposals = st.session_state.setdefault("_change_proposals", {})
    proposals.update(result.data.get("results", {}))
    skipped = result.data.get("skipped", [])
    if skipped:
        st.session_state["_change_proposal_msg"] = "対象外: " + ", ".join(
            f"#{sn}（{msg}）" for sn, msg in skipped
        )


def apply_all_change_proposals():
    """⑤変更比較：保持中の全ステージ変更案を実データに一括反映する。"""
    proposals = st.session_state.get("_change_proposals") or {}
    if not proposals:
        return
    edited_by_stage = {}
    for stage_no, pdata in proposals.items():
        edited_by_stage[stage_no] = _collect_resolved_ops_for_stage(stage_no, pdata)
    result = _image_wf.apply_all_change_proposals(
        app_state, st.session_state.diff_tgt_event,
        st.session_state.diff_tgt_img_type,
        st.session_state.get("uploaded_image_updated"),
        edited_by_stage,
    )
    if not result.success:
        st.warning(result.error)
        return
    # 反映済み状態をクリアし、差分結果も無効化
    st.session_state["_change_proposals"] = {}
    st.session_state["_diff_result"] = None
    st.session_state["_change_proposal_msg"] = "変更を反映しました。⑥を開くとNULLのIDが採番されます。"
    _sync_to_session(app_state)
...
""")
    ...
    # 事前に session_state を初期化し index= は渡さない (④と同じパターン)
    if st.session_state.get("crop_tgt_event") not in event_list:
        st.session_state.crop_tgt_event = (
            app_state.crop.crop_tgt_event
            if app_state.crop.crop_tgt_event in event_list
            else event_list[0]
        )
    st.selectbox("イベント", event_list, key="crop_tgt_event", on_change=set_crop_image)
    ...
    if len(event_type_list) == 0:
        ...
        return
    if st.session_state.get("crop_tgt_img_type") not in event_type_list:
        st.session_state.crop_tgt_img_type = (
            app_state.crop.crop_tgt_img_type
            if app_state.crop.crop_tgt_img_type in event_type_list
            else event_type_list[0]
        )
    st.selectbox("種別", event_type_list, key="crop_tgt_img_type", on_change=set_crop_image)
...
""")
                    st.slider('エッジ抽出の閾値', value=285, min_value=1, max_value=500, step=1, key="x_edge_threshold_2", on_change=detect_stageline,args=(app_state.crop.cropped_image,))
                    st.slider('抽出したエッジを伸ばす際の閾値', value=130, min_value=1, max_value=500, step=1, key="x_edge_threshold_1", on_change=detect_stageline,args=(app_state.crop.cropped_image,))
                    st.slider('ハフ変換の閾値', value=100, min_value=1, max_value=500, step=1, key="x_hough_threshold", on_change=detect_stageline,args=(app_state.crop.cropped_image,))
                    st.slider('ハフ変換で許容する線分の飛び', value=1, min_value=0, max_value=100, step=1, key="x_hough_gap", on_change=detect_stageline,args=(app_state.crop.cropped_image,))
                    st.slider('抽出線分の長さ（元画像の縦に対する比率）', value=0.01, min_value=0.0, max_value=1.0, step=0.001, key="x_minlength_rate", on_change=detect_stageline,args=(app_state.crop.cropped_image,))
                    st.slider('同一視する線分の許容誤差幅', value=5, min_value=1, max_value=30, step=1, key="x_identify_interval", on_change=detect_stageline,args=(app_state.crop.cropped_image,))
            with split_button[1]:
                st.info(
"""###### 均等幅での分割
...
- 読み取り済みのタイムテーブルが変更になった場合に、新旧画像の差分を検出し、差分のあるステージについてLLMで変更案を生成・編集して反映できます
- 「差分画像を出力する」→ 差分のありそうなステージで「変更案を生成」→ 内容を確認・修正 →「全ステージの変更を反映」の順で操作します""")
    event_list = get_event_name_list()
    ...
    # 変更案の生成/反映に関する一過性メッセージ（表示後に消去し、残り続けないようにする）
    msg = st.session_state.pop("_change_proposal_msg", None)
    if msg:
        st.info(msg)
...
def _render_diff_result(diff_result):
    ...
    # タイテ(stage_n.json)が有るステージのみ変更案の生成対象
    eligible = [s for s in diff_stages if _stage_json_exists(s["stage_no"])]
    if eligible:
        st.button(
            f"差分ありの全ステージに変更案を生成（{len(eligible)}件）",
            on_click=generate_change_proposal_all, args=(eligible,),
            type="primary", key="_cp_gen_all",
        )
        _render_change_rule_help()

    proposals = st.session_state.get("_change_proposals") or {}
    for s in diff_stages:
        stage_no = s["stage_no"]
        n_regions = len(s.get("regions") or [])
        st.markdown(f"###### #{stage_no} {s['stage_name']}（{s['score']:.2f}% / 差分領域 {n_regions}件）")
        cols = st.columns(2)
        with cols[0]:
            st.caption("既存")
        ...
        if not _stage_json_exists(stage_no):
            st.caption("タイテ未作成のため変更案の対象外です。")
            continue
        st.button(
            "このステージの変更案を生成", key=f"_cp_gen_{stage_no}",
            on_click=generate_change_proposal_for_stage, args=(stage_no,),
        )
        if stage_no in proposals:
            _render_stage_proposal(stage_no, proposals[stage_no])

    if proposals:
        st.divider()
        st.caption("人手修正を反映した上で、全ステージの変更をまとめて実データへ反映します（raw画像も差し替え）。")
        st.button(
            "全ステージの変更を反映", on_click=apply_all_change_proposals,
            type="primary", key="_cp_apply_all",
        )


def _stage_json_exists(stage_no) -> bool:
    """対象イベント/種別の stage_n.json が存在するか。"""
    return os.path.exists(os.path.join(
        app_state.project.pj_path,
        st.session_state.diff_tgt_event,
        st.session_state.diff_tgt_img_type,
        f"stage_{stage_no}.json",
    ))


def _render_change_rule_help():
    """グループ名・グループID・出番IDの反映ルールの説明を表示する。"""
    with st.expander("ℹ️ グループ名・ID の反映ルール（クリックで展開）"):
        st.markdown("""
**グループ名（編集欄の値）= 生の読み取り(raw)**
- 入力欄は「既存のグループ名（採用マスタ）から選択」と「新規の自由入力」の両方が可能です。
- 編集欄に表示・入力する「グループ名」は OCR相当の生の読み取りとして `グループ名` に保存されます。
- 名称を変更したレコードは、`グループ名`(raw) も新規画像の読みで更新します（時刻だけの変更では raw は変わりません）。

**グループ名_採用（正式名）/ グループID … 反映時に下表のルールで決まります**

| ケース | グループ名(raw) | グループ名_採用（正式名） | グループID |
|---|---|---|---|
| 新規・再採番（マスタ未一致） | 入力値 | **反映時に補正（マスタ照合）で自動生成** | NULL（⑥で新規採番） |
| 既存マスタに一致 | 入力値 | その一致名を採用 | 一致したマスタのID |
| グループID保持の改名 | 入力値 | 入力値を採用（**マスタ側の名称も更新**） | 現状維持 |
| 時刻のみ変更 | 不変 | 不変 | 現状維持 |

**出番ID**
- `保持`＝現在のIDを維持 / `再採番(NULL)`＝NULL化して⑥で採番 / `手入力`＝指定値。
- 既定値は自動判定：グループ名変更→再採番、コラボ（同一出番IDが複数）の部分変更→再採番、同名・時刻のみ変更→保持。

NULL化したIDは、**⑥を開いたタイミングで採番**されます。
""")


def _render_stage_proposal(stage_no, pdata):
    """1ステージの変更案を「元(読取専用)＋編集欄」の2段構成で描画する。"""
    proposal = pdata.get("proposal") or {}
    ops = proposal.get("操作") or []
    with_tk = pdata.get("with_tokutenkai", False)
    timetable = (pdata.get("stage_json") or {}).get("タイムテーブル", [])
    name_to_gid = pdata.get("name_to_group_id") or {}
    known_groups = pdata.get("known_groups") or []

    if not ops:
        st.success("変更は不要と判断されました（変更案なし）。")
        return
    ...
    with st.container(border=True):
        for oi, op in enumerate(ops):
            kp = f"_cp_{stage_no}_{oi}"
            kind = op.get("種別")
            head = st.columns([3, 1])
            head[0].markdown(f"**操作{oi + 1}: {kind}**")
            head[1].checkbox("反映する", value=True, key=f"{kp}_apply")
            if op.get("理由"):
                st.caption(f"理由: {op['理由']}")
            if kind == "変更":
                _render_change_op(kp, op, timetable, with_tk, turn_counts, name_to_gid, known_groups)
            elif kind == "削除":
                _render_delete_op(kp, op, timetable, with_tk)
            elif kind == "追加":
                _render_add_op(kp, op, with_tk, name_to_gid, known_groups)
            st.divider()


def _group_name_input(container, key, default, known_groups):
    """既存グループ名（採用マスタ）からの選択と自由入力を両立する入力欄。

    Streamlit 1.49+ の selectbox(accept_new_options=True) を利用。
    default が選択肢に無くても先頭に差し込んで初期値にできるようにする。
    """
    default = default or ""
    opts = list(dict.fromkeys(([default] if default else []) + list(known_groups or [])))
    return container.selectbox(
        ...
        key=key,
        placeholder="既存から選択 または 新規入力",
    )


def _render_change_op(kp, op, timetable, with_tk, turn_counts, name_to_gid, known_groups):
    idx = op.get("対象連番")
    rec = timetable[idx] if (idx is not None and 0 <= idx < len(timetable)) else {}
    cur_turn = rec.get("出番ID")
    cur_gid = rec.get("グループID")
    cur_name = rec.get("グループ名_採用") or rec.get("グループ名") or ""
    cur_live = rec.get("ライブステージ") or {}
    st.caption(
        f"元: 連番{idx} / 出番ID={cur_turn} / グループID={cur_gid} / "
        f"{cur_name} / {cur_live.get('from')}-{cur_live.get('to')}"
    )

    prop_name = op.get("グループ名") or ""
    prop_live = op.get("ライブ時間") or {}
    c = st.columns([2, 1, 1])
    new_name = _group_name_input(c[0], f"{kp}_name", (prop_name or cur_name), known_groups)
    c[1].text_input("from", value=(prop_live.get("from") or cur_live.get("from") or ""), key=f"{kp}_from")
    c[2].text_input("to", value=(prop_live.get("to") or cur_live.get("to") or ""), key=f"{kp}_to")

    is_collab = cur_turn is not None and turn_counts.get(cur_turn, 0) > 1
    eff_name_changed = bool(new_name) and new_name != cur_name
    turn_default = "再採番(NULL)" if (eff_name_changed or is_collab) else "保持"
    matched_gid = name_to_gid.get(new_name) if eff_name_changed else None

    pc = st.columns(3)
    turn_opts = ["保持", "再採番(NULL)", "手入力"]
    pc[0].radio("出番ID", turn_opts, index=turn_opts.index(turn_default),
                key=f"{kp}_turnpol", horizontal=True)
    pc[1].number_input("出番ID(手入力)", value=int(cur_turn) if cur_turn is not None else 0,
                       step=1, key=f"{kp}_turnval")
    gid_opts = ["保持", "再採番(NULL)"]
    if matched_gid is not None:
        gid_opts.append(f"既存マスタID={matched_gid}")
        gid_default = gid_opts[-1]
    elif eff_name_changed:
        gid_default = "再採番(NULL)"
    else:
        gid_default = "保持"
    pc[2].radio("グループID", gid_opts, index=gid_opts.index(gid_default),
                key=f"{kp}_gidpol", horizontal=True)

    if with_tk:
        _render_tokutenkai_ops_editor(kp, op.get("特典会操作") or [])


def _render_delete_op(kp, op, timetable, with_tk):
    idx = op.get("対象連番")
    rec = timetable[idx] if (idx is not None and 0 <= idx < len(timetable)) else {}
    del_kind = op.get("削除種別") or "全体"
    name = rec.get("グループ名_採用") or rec.get("グループ名") or ""
    if with_tk and del_kind == "特典会のみ":
        st.caption(f"元: 連番{idx} / {name} の特典会[{op.get('特典会連番')}] を削除")
    else:
        st.caption(f"元: 連番{idx} / 出番ID={rec.get('出番ID')} / {name} をレコードごと削除")


def _render_add_op(kp, op, with_tk, name_to_gid, known_groups):
    prop_name = op.get("グループ名") or ""
    prop_live = op.get("ライブ時間") or {}
    c = st.columns([2, 1, 1])
    new_name = _group_name_input(c[0], f"{kp}_name", prop_name, known_groups)
    c[1].text_input("from", value=prop_live.get("from") or "", key=f"{kp}_from")
    c[2].text_input("to", value=prop_live.get("to") or "", key=f"{kp}_to")
    matched = name_to_gid.get(new_name)
    if matched is not None:
        st.caption(f"既存マスタにグループ名一致: グループID={matched} を使用します。")
    else:
        st.caption("新規グループ（グループID・出番IDは⑥で採番されます）。")
    if with_tk:
        _render_tokutenkai_ops_editor(kp, op.get("特典会操作") or [], add_only=True)


def _render_tokutenkai_ops_editor(kp, tk_ops, add_only=False):
    if not tk_ops:
        return
    st.caption("特典会の変更/追加")
    for ti, t in enumerate(tk_ops):
        sub = t.get("操作種別") or ("追加" if add_only else "変更")
        tc = st.columns([1, 1, 1, 1])
        label = sub + (f"[{t.get('対象特典会連番')}]" if sub == "変更" else "")
        tc[0].text_input("操作", value=label, key=f"{kp}_tk{ti}_label", disabled=True)
        tc[1].text_input("ブース", value=t.get("ブース") or "", key=f"{kp}_tk{ti}_booth")
        tc[2].text_input("特from", value=t.get("from") or "", key=f"{kp}_tk{ti}_from")
        tc[3].text_input("特to", value=t.get("to") or "", key=f"{kp}_tk{ti}_to")


# --- 変更案の収集（widget state → 解決済み操作） ---

def _collect_resolved_ops_for_stage(stage_no, pdata):
    proposal = pdata.get("proposal") or {}
    ops = proposal.get("操作") or []
    with_tk = pdata.get("with_tokutenkai", False)
    timetable = (pdata.get("stage_json") or {}).get("タイムテーブル", [])
    ...
    for oi, op in enumerate(ops):
        kp = f"_cp_{stage_no}_{oi}"
        if not st.session_state.get(f"{kp}_apply", True):
            continue
        kind = op.get("種別")
        if kind == "変更":
            r, upd = _collect_change_op(kp, op, timetable, with_tk, pdata)
            resolved.append(r)
            master_name_updates.update(upd)
        elif kind == "削除":
            resolved.append(_collect_delete_op(op))
        elif kind == "追加":
            resolved.append(_collect_add_op(kp, op, with_tk, pdata))
    return {
        "with_tokutenkai": with_tk,
        "resolved_ops": resolved,
        "master_name_updates": master_name_updates,
    }


def _collect_change_op(kp, op, timetable, with_tk, pdata):
    idx = op.get("対象連番")
    rec = timetable[idx] if (idx is not None and 0 <= idx < len(timetable)) else {}
    cur_name = rec.get("グループ名_採用") or rec.get("グループ名") or ""
    cur_gid = rec.get("グループID")
    new_name = st.session_state.get(f"{kp}_name") or cur_name
    new_from = st.session_state.get(f"{kp}_from", "")
    new_to = st.session_state.get(f"{kp}_to", "")
    eff_name_changed = bool(new_name) and new_name != cur_name

    turnpol = st.session_state.get(f"{kp}_turnpol", "保持")
    if turnpol == "保持":
        set_turn = _diff_llm.KEEP
    elif turnpol == "再採番(NULL)":
        set_turn = None
    else:
        set_turn = int(st.session_state.get(f"{kp}_turnval", 0))

    gidpol = st.session_state.get(f"{kp}_gidpol", "保持")
    master_updates = {}
    set_raw = None
    set_adopted = None
    needs_correction = False
    if eff_name_changed:
        set_raw = new_name  # 別グループへの変更等。rawも新規画像の読みで更新
        if gidpol == "保持":
            set_gid = _diff_llm.KEEP
            set_adopted = new_name  # 同一グループの改名扱い → 採用名も更新
            if cur_gid is not None:
                master_updates[cur_gid] = new_name  # マスタ側の名称も更新
        elif gidpol.startswith("既存マスタID="):
            set_gid = int(gidpol.split("=")[1])
            set_adopted = new_name  # マスタ完全一致 → その採用名を使用
        else:  # 再採番(NULL): 新規/不明グループ
            set_gid = None
            needs_correction = True  # 採用名は補正ステップ（idolname埋め込み等）で生成
    else:
        # 時刻のみ変更等：グループ名/採用名は変更しない
        if gidpol == "再採番(NULL)":
            set_gid = None
        elif gidpol.startswith("既存マスタID="):
            set_gid = int(gidpol.split("=")[1])
    ...
    resolved = {
        "種別": "変更",
        "対象連番": idx,
        "set_グループ名_raw": set_raw,
        "set_グループ名_採用": set_adopted,
        "needs_correction": needs_correction,
        "set_ライブ時間": {"from": new_from, "to": new_to},
        "set_出番ID": set_turn,
        "set_グループID": set_gid,
    }
    if with_tk:
        resolved["特典会操作"] = _collect_tokutenkai_ops(kp, op.get("特典会操作") or [])
    return resolved, master_updates


def _collect_delete_op(op):
    return {
        "種別": "削除",
        "対象連番": op.get("対象連番"),
        "削除種別": op.get("削除種別") or "全体",
        "特典会連番": op.get("特典会連番", -1),
    }


def _collect_add_op(kp, op, with_tk, pdata):
    name_to_gid = pdata.get("name_to_group_id") or {}
    new_name = st.session_state.get(f"{kp}_name") or (op.get("グループ名") or "")
    new_from = st.session_state.get(f"{kp}_from", "")
    new_to = st.session_state.get(f"{kp}_to", "")
    matched = name_to_gid.get(new_name)
    if matched is not None:
        set_gid = matched
        set_adopted = new_name  # マスタ完全一致 → その採用名・グループIDを使用
        needs_correction = False
    else:
        set_gid = None
        set_adopted = None
        needs_correction = True  # 採用名は補正ステップで生成
    resolved = {
        "種別": "追加",
        "対象連番": -1,
        "set_グループ名_raw": new_name,
        "set_グループ名_採用": set_adopted,
        "needs_correction": needs_correction,
        "set_ライブ時間": {"from": new_from, "to": new_to},
        "set_出番ID": None,
...
def _collect_tokutenkai_ops(kp, tk_ops):
    out = []
    for ti, t in enumerate(tk_ops):
        sub = t.get("操作種別") or "変更"
        out.append({
            "操作種別": sub,
            "対象特典会連番": t.get("対象特典会連番", -1),
            "ブース": st.session_state.get(f"{kp}_tk{ti}_booth", t.get("ブース")),
            "from": st.session_state.get(f"{kp}_tk{ti}_from", t.get("from")),
            "to": st.session_state.get(f"{kp}_tk{ti}_to", t.get("to")),
        })
    return out
